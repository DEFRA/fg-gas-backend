...
describe("compareDesc / mergePages", () => {
  ...
  it("answers with the same page in the journey's shape", () => {
    const pages = [
      page("gasInbox", [tuple("gasInbox", 1)]),
      page("cwInbox", [tuple("cwInbox", 2)]),
    ];

    const { events, hops } = mergePages({ pages, direction: "forward" });

    expect(hops.map((hop) => hop.id)).toEqual(events.map((event) => event.id));
    expect(hops.map((hop) => hop.startedAt)).toEqual([
      "2026-06-16T10:02:00.000Z",
      "2026-06-16T10:01:00.000Z",
    ]);
  });
...
describe("buildPagination", () => {
  ...
  // cursors - see unconsumedSlice in merge-event-pages.js.
