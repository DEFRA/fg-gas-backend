...
describe("findEventsQuerySchema", () => {
  ...
  it("accepts a cursor, a status and a service", () => {
    ...
    expect(value).toEqual({
      ...
      status: "DEAD_LETTER",
      service: "caseworking",
      // Defaulted, exactly as `direction` is: an operator scanning for stuck
      // work is not looking for the reads somebody did.
      audit: "exclude",
    });
  });
...
describe("findEventsQuerySchema q", () => {
  ...
  // `kind` is not an accepted parameter, so it is a 400, not a silent ignore.
...
describe("findEventsQuerySchema error", () => {
  ...
  // The same ceiling the store keeps `lastError.message` under: the panel
  // must never offer a group the list refuses as a filter.
  it("takes a message as long as the store can hold, and no longer", () => {
    expect(
      findEventsQuerySchema.validate({ error: "x".repeat(1024) }).error,
    ).toBeUndefined();
    expect(
      findEventsQuerySchema.validate({ error: "x".repeat(1025) }).error,
    ).toBeDefined();
  });
