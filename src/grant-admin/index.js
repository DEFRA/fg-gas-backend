...
export const grantAdmin = {
  name: "grant-admin",
  register(server) {
    // The single-segment routes (`/counts`, `/breakdown`) are registered
    // before the three-segment `/{service}/{box}/{id}` ones. They cannot
    // collide - the segment counts differ - but the order keeps the list
    // readable, and index.test.js asserts each path reaches the route it
    // names.
    server.route([
      getClaimsRoute,
      findEventsRoute,
