...
describe("agreementDefinitionSchema resolver instructions", () => {
  ...
  it("accepts explicit grid, form and button action references", () => {
    const definition = withComponents([
      {
        component: "grid-row",
        components: [
    ...
    expect(validate(definition).error).toBeUndefined();
  });

  it.each([
    [
      "a grid column with an unsupported width",
      [
        {
          component: "grid-row",
          components: [
            {
              component: "grid-column",
              width: "quarter",
              components: [{ component: "paragraph", text: "x" }],
            },
          ],
        },
      ],
  ...
  ])("rejects %s", (_name, components, expected) => {
    expect(messagesFor(withComponents(components))).toMatch(expected);
  });
