...
describe("find available claims use case", () => {
  ...
  it("omits decimal constraints for string fields", async () => {
    ...
    expect(result.availableClaims[0].data.actionCode.minValue).toBeUndefined();
  });
  ...
  it("returns multiple available claims when multiple entitlements exist", async () => {
    ...
    findExistingEntitlements.mockResolvedValue([
      createEntitlement({
        data: { totalHectares: 100, actionCode: "PA3", actionVersion: "1.0.0" },
      }),
      createEntitlement({
        data: { totalHectares: 200, actionCode: "PA3", actionVersion: "1.0.0" },
      }),
    ]);
