...
Payment cutover remains subject to the applicable gates below. [LDR-005](./ldr-005-development-payment-compatibility-scope.md) records separately why legacy Agreement scheme parity is not applicable to the current development-only cutover; it does not approve live traffic or waive the Payment Service contract and deployment gates:
