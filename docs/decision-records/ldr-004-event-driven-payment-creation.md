...
A one-off local comparison ran the legacy FPTT mapper and SNS serializer against an Agreement with two scheduled payments and both parcel-level and Agreement-level invoice lines, then built and mapped the equivalent GAS Payment. After normalising only generated event IDs, event times and due-payment correlation IDs, the complete CloudEvent payloads were structurally identical. The captured event remains a serializer regression. FPTT has since closed and is not a migrated Agreement scheme, so its source-specific mapping is historical evidence rather than a live cutover gate.
...
- Every Payment definition used for a migrated scheme is checked against its supported Payment Hub interface. The current `pigs-might-fly` definition preserves `scheme: SFI`, `sourceSystem: FPTT`, `deliveryBody: RP00`, `fesCode: FALS_FPTT`, `ledger: AP`, invoice-line `accountCode: SOS710`, `fundCode: DRD10`, invoice-line `deliveryBody: RP00`, marketing-year derivation and stringified monetary values. Configurability must not silently change that contract. The closed FPTT grant is excluded from migration scope.
...
For every migrated Agreement scheme, compatibility proof must exercise the published event through the same serializer and SNS adapter used at runtime. A hand-authored fixture or a mapper-only unit test is not sufficient evidence of transport compatibility. Closed schemes may remain as historical serializer fixtures without blocking cutover.
...
This record defines the intended architecture but does not by itself prove compatibility for a migrated scheme. Definition-specific cutover evidence is required before that scheme's live traffic moves; closed schemes outside migration scope do not block the cutover.
