...
describe("findEventsUseCase with CW_BACKEND_* unset", () => {
  it("returns GAS rows with two not configured sourceErrors and makes no HTTP call", async () => {
    ...
    // The use case also answers with `hops`, which the endpoint does not
    // publish, so only the three published keys are checked.
    const { events, pagination, sourceErrors } = page;

    expect(
      findEventsResponseSchema.validate({ events, pagination, sourceErrors })
        .error,
    ).toBeUndefined();
    expect(page.hops.map((hop) => `${hop.service}/${hop.box}`)).toEqual([
      "gas/outbox",
      "gas/inbox",
    ]);
  });
