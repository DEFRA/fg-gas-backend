...
const eventsPage = (query) =>
  wreck.get(
    query
      ? `/grant-admin/events/page?${new URLSearchParams(query)}`
      : "/grant-admin/events/page",
  );

const breakdown = async (query) => {
  const { payload } = await eventsPage(query);

  return { payload: payload.breakdown };
};
...
describe("GET /grant-admin/events/page - the breakdown section", () => {
  ...
  it("counts only DEAD_LETTER rows", async () => {
    ...
    await inbox.insertMany([
      ...
      aLiveInboxDoc({
        segregationRef: ref,
        status: "PROCESSING",
        claimExpiresAt: new Date(2099, 0),
      }),
    ]);
  ...
  it("merges the box Caseworking could read and names only the one it could not", async () => {
    const ref = seg();
    await inbox.insertOne(aDeadInboxDoc({ segregationRef: ref }));
    await setCwStub({
      inbox: {
  ...
  it("ignores an error filter - the breakdown already answers that question", async () => {
    const ref = seg();
    await inbox.insertMany([
      aDeadInboxDoc({ segregationRef: ref }),
      aDeadInboxDoc({
    ...
    const { payload } = await breakdown({ q: ref, error: "boom" });

    expect(payload.groups.map((group) => group.error).sort()).toEqual([
      "No handler found",
      "boom",
    ]);
  });
  ...
  it("reaches the page route rather than the detail route", async () => {
    const { payload } = await breakdown({});
