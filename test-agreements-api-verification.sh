...
# Test script for Agreement Service message provider verification
...
curl -u "${PACT_USER}:${PACT_PASS}" \
  "${PACT_BROKER_BASE_URL}/pacts/provider/fg-gas-backend/consumer/farming-grants-agreements-api/latest" \
  -o tmp/pacts/farming-grants-agreements-api-fg-gas-backend.json
...
for pact_file in tmp/pacts/farming-grants-agreements-api-fg-gas-backend*.json; do
  if [ -f "$pact_file" ]; then
